...
#'
#' @return returns a dataframe with two columns - `team`, `sos`
#'
  ...
  function(ratings, schedule) {
    ...
    opp <-
      ...
      dplyr::mutate(
        rating =
          dplyr::if_else(
            .data$home_or_away == "Away",
            .data$rating + (abs(.data$rating) * 0.25),
            .data$rating
            )
      ) |>
    ...
    opp_opp <-
      ...
      dplyr::summarise(
        opponent_opponent_rating = mean(.data$rating), .by = .data$team
        )
